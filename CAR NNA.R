...
d<- read_csv("1.1.1 BdD_CAR_NNA_3.csv") %>%  
  clean_names()
...
assert_all_are_in_past(d2$FECHA.DE.REPORTE.DE.LA.INFORMACIÓN)
...
d1 <- d1 %>%
  mutate(Fecha_reporte = parse_date_time(FECHA.DE.REPORTE.DE.LA.INFORMACIÓN, formats))
...
theme_r <- theme(axis.line.y = element_blank(),
